...
int testval;	
	...
	// a[3]="d";
recnum=0;
AM_InsertEntry(fd,INT_TYPE,sizeof(int),"b",1); 
		printf("%d\n",recnum );	/* first, make sure that simple deletions work */
recnum++;
int x=5;
AM_InsertEntry(fd,INT_TYPE,sizeof(int),(char *)&x,100);
		printf("%d\n",recnum );	/* first, make sure that simple deletions work */
recnum++;

AM_InsertEntry(fd,INT_TYPE,sizeof(int),"a",recnum);
		printf("%d\n",recnum );	/* first, make sure that simple deletions work */
recnum++;

AM_InsertEntry(fd,INT_TYPE,sizeof(int),"d",recnum);
		printf("%d\n",recnum );	/* first, make sure that simple deletions work */
recnum++;

AM_InsertEntry(fd,INT_TYPE,sizeof(int),"e",recnum);
		printf("%d\n",recnum );	/* first, make sure that simple deletions work */
	...
	printf("retrieving  number records\n");
	numrec= 0;
	...
	for (recnum=0; recnum < 4; recnum += 2)
		AM_DeleteEntry(fd,INT_TYPE,sizeof(int),(char *)&recnum,
					recnum);
